...
const NEETCODE_150_SLUGS = [
  ...
  "hand-of-straights", "largest-number", "partition-labels",
  // Intervals
...
const main = async () => {
  const unique = [...new Set(NEETCODE_150_SLUGS)];
  ...
  const missing = unique.filter(s => !foundSlugs.includes(s));

  console.log(`Found: ${found.length} / ${unique.length}`);
  if (missing.length) console.log("Missing from DB:", missing);

  await prisma.problem.updateMany({
    where: { slug: { in: foundSlugs } },
    data: { isNeetCode150: true },
  });
